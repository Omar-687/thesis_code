...
def mpe_cost_graph(
    ...
    for i in range(2):
        ...
        if i == 0 and mpe_values_env is not None and cost_values_env is not None:
            ...
            costs_of_given_alg = cost_values_env / num_of_days
            ...
            costs_of_given_alg = cost_values_offline / num_of_days
...
def plot_departures_for_given_day(evs, day, period):
    ...
    # Show the plot
    plt.show()
def convert_mwh_to_kw_prices(prices, time_between_timesteps):
    return (prices / 1000) * (time_between_timesteps / 60)
...
def create_table(charging_profiles_matrix,
    ...
    for i in range(start_hour, end_hour):
        ...
        if show_charging_costs:
            charging_ev_cost = (charging_rates/(period/60)) @ current_prices
    ...
    if show_charging_costs:
        ...
        data[f'({start_hour}-{end_hour})'] = (list((charging_profiles_matrix/(period/60)) @
