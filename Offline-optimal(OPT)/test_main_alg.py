...
class moretestsRL(unittest.TestCase):
    ...
    def test_run_model_on_testing_env(self):
        ...
        for _ in range(steps_per_episode*k_days):
            ...
            if terminated or truncated:
                ut_signals = eval_env.signal_ut
                cumulative_charging_rates = eval_env.chosen_sum_of_charging_rates

                current_mpe_error = mpe_error_fun_rl_testing(ev_diction=eval_env.delivered_and_undelivered_energy)
                mpes.append(current_mpe_error)
                current_mse_error = mse_error_fun_rl_testing(sum_of_charging_rates=cumulative_charging_rates,
                                                             ut_signals=ut_signals,
                                                             capacity_constraint=operator_constraint)
                mses.append(current_mse_error)
                # charging_
    ...
    def testRLourenvlibrary(self):
        ...
        hyp_diction = {'Reward':'o1*sum_{i in active evs}||s_t(i)|| - o2* sum_{i in active evs} I(t = d(i))[e_{t}(j)]',
                       'o1':1,
                       'o2':0.5}
