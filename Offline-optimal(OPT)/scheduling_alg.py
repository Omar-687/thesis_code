...
class SchedulingAlg:
    def __init__(self,
        ...
        if not set_time_horizon:
            self.time_horizon = []
        if set_power_limit_for_each_timestep and not isinstance(power_limit, list) and not isinstance(power_limit, int) and not isinstance(power_limit, float):
            raise ValueError('Power limit must be either list, int or float.')

        if set_power_limit_for_each_timestep and isinstance(power_limit, list):
            if len(power_limit) != len(self.time_horizon):
                raise ValueError('Power limit list must be of same length as time horizon.')
            self.power_limit = power_limit

        if (set_power_limit_for_each_timestep and
                (isinstance(power_limit, int) or isinstance(power_limit, float))):
            # if available_energy_for_each_timestep <= 0:
            #     raise ValueError('Available energy must be above 0.')
            self.power_limit = np.zeros(
                shape=(len(self.time_horizon))) + power_limit
        if not set_power_limit_for_each_timestep:
            self.power_limit = []
